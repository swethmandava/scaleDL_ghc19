...
# Create model
def initialize_model(nb_users, nb_items):
    # make pytorch memory behavior more consistent later
    torch.cuda.empty_cache()
    if DefaultArgs.seed is not None:
        torch.manual_seed(DefaultArgs.seed)
    model = NeuMF(nb_users, nb_items,
                  mf_dim=DefaultArgs.factors, mf_reg=0.,
                  mlp_layer_sizes=DefaultArgs.layers,
                  mlp_layer_regs=[0. for i in DefaultArgs.layers],
                  dropout=DefaultArgs.dropout)
    model = model.cuda()
    print(model)
